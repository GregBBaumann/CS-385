/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Gregory Baumann, Roma Razdan
 * Version     : 1.0
 * Date        : 11/23/20
...
//    					left_rotate(z->parent->parent);
    ...
    int diameter(Node<K, V> *node) const {
    	...
    	if(node == nullptr){
    		...
    		int maxnodediameter = (height(node->left)+1) + (height(node->right)+1) + 1;
    		if (diameterleft > diameterright && diameterleft > maxnodediameter){
    			return diameterleft;
    		}else if (diameterright > maxnodediameter){
    			return diameterright;
    		}else{
    			return maxnodediameter;
    		}
    	}
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
    	...
    	int sum = 0;
    	int levels = 0;
    	int heights = height(node);
    	while(levels - 1 != heights){
    		sum = sum + (levels * width(node, levels));
    		levels++;
    	}
    	return sum;
    }
    ...
    size_t sum_null_levels(Node<K, V> *node, size_t level) const {
    	...
    	int heights = height(node);
    	int levels = 1;
    	while(levels - 1 != heights + 1){
    		int levelmax = (width(node, levels-1)*2);
    		sum = sum + ((levelmax - width(node, levels))*(levels));
    		levels++;
    	}
    	return sum;
    }
